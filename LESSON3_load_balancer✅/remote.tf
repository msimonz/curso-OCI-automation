...
resource "null_resource" "msimonzWebserverHTTPD" {
  ...
  depends_on = [oci_core_instance.msimonzWebserver]
  provisioner "remote-exec" {
    connection {
      type        = "ssh"
      user        = "opc"
      host        = data.oci_core_vnic.msimonzWebserver_VNIC1[count.index].public_ip_address
    ...
    inline = ["echo '== 1. Installing HTTPD package with dnf'",
      ...
      "sudo /bin/su -c \"echo 'Welcome to msimonz.com! This is WEBSERVER${count.index + 1}...' > /var/www/html/index.html\"",
